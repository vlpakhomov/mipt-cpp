...
python3 banned_words_checker.py --solution=../include/string/$1.hpp --bannded-words=banned_words.json
if [[ ! $? -eq 0 ]]
then 
...
clang-tidy -extra-arg=-std=c++17 -quiet ../include/string/$1.hpp
if [[ ! $? -eq 0 ]]
then 
...
#clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' ../include/string/$1.hpp &&
#clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' ../src/$1.cpp &&
#git diff --ignore-submodules --color ../src/$1.cpp ../include/string/$1.hpp > diff &&
#cat diff
#if [[ -s diff ]]
#then 
