...
1. Решение должно состоять из двух файлов: `matrix.hpp` с объявлением класса и его методов и `matrix.cpp` с определением всего необходимого. 

2. В данной задаче запрещено использовать слова static_assert и constexpr, если вдруг вы такое знаете.

3. В данной задаче разрешено использовать `std::vector<T>`.
